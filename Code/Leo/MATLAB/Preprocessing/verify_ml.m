function verify_ml(depth_frames, mov, qtm_data, fcss_data, fs, theta_type, i_time, save_stat, save_vid_file, ml_data)
    % load qtm data 
    ...
    if strcmp(theta_type, 'all_raw') == 1
        ...
        xlabel('time(s)');
        legend(ax1, 'Fz1', 'Fz2', 'Fz3', 'Fx1', 'Fx2', 'Fy', 'Location', 'northwestoutside');
        grid on;
    end
    
    
    ax_vid = subplot(1, n_subplots, 2);
...
%     curve13 = animatedline(ax3, 'LineWidth', 2, 'Color', [0.9290, 0.6940, 0.1250]);
    curve14 = animatedline(ax3, 'LineWidth', 2, 'Color', [0, 0.4470, 0.7410], 'LineStyle', '--');
%     curve15 = animatedline(ax3, 'LineWidth', 2, 'Color', [0.8500, 0.3250, 0.0980], 'LineStyle', '--');
%     curve16 = animatedline(ax3, 'LineWidth', 2, 'Color', [0.9290, 0.6940, 0.1250], 'LineStyle', '--');
...
%         legend(ax3, '\theta_x', '\theta_y', '\theta_z', '\theta_x pred', '\theta_y pred', '\theta_z pred', 'Location', 'northeastoutside');
        legend(ax3, '\theta_z', '\theta_z pred', 'Location', 'northeastoutside');
    end
...
%             addpoints(curve11, time_idx(i), theta_interest(i,3));%theta_x
%             addpoints(curve12, time_idx(i), theta_interest(i,2));%theta_y
            addpoints(curve11, time_idx(i), theta_interest(i,1));  % theta_z
            addpoints(curve14, time_idx(i), ml_data(i,3));
%             addpoints(curve15, time_idx(i), ml_data(i,2));
%             addpoints(curve16, time_idx(i), ml_data(i,1));
