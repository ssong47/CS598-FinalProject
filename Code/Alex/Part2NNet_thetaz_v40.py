...
callback2= tf.keras.callbacks.EarlyStopping(monitor='val_loss',patience=10,restore_best_weights=True)

save_notebookparams = 1
if save_notebookparams:
    pkl_filename = "depthforcemodelparam_cnn3v40_pb.pkl"
    randata = {}
...
for e in range(epochs):
    ...
    history=model.fit([Xaug, X2], y, epochs=1, callbacks=[callback, callback2], validation_data = ([Xvalid/255.,Xvalid2], y_valid),batch_size=batch_size, verbose=2)

    acc = history.history['val_mae'][0]
    if acc>accbest:
        accbest=acc
        model.save('cnn3v40')
        print('Saved new parameter at {:5.2f} degrees'.format(acc))
